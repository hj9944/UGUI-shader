    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 PixShader(pixel_t input) : SV_Target
            {
                ...
                float2 noiseUV = float2(input.srcPos.xy * _Ramp_ST + _Ramp_ST.zw);
                float cutout = tex2D(_Ramp, noiseUV).r; 

                half d = tex2D(_MainTex, input.texcoord0.xy).a * input.param.x;
                half4 c = input.faceColor * saturate(d - input.param.w);

                half4 cc = tex2D(_MainTex, input.texcoord0.xy);

                cc.a *= smoothstep(0, 0.1, _Blend - cutout);

                c = input.faceColor * cc.a * input.param.x * step(0, d - input.param.w);
                // c.a *= step(0, _Blend - cutout);
